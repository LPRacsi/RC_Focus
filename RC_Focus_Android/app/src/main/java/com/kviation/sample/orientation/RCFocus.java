...
public class RCFocus extends View {
  ...
  private final Paint mShift, mShiftFill, clearPaint;
  private int shiftGear, shiftGearPrev;
  private String shiftGearString;
  ...
  private int mWidth;
  private int mHeight;
  private int mAngleChordX = 5;
  private int mAngleChordY = 45;

  private float mPitch = 0; // Degrees
  private float mRoll = 0; // Degrees, left roll is positive

  public RCFocus(Context context) {
    this(context, null);
  }

  public RCFocus(Context context, AttributeSet attrs) {
    super(context, attrs);
    ...
    mShift = new Paint();
    mShift.setColor(Color.WHITE);
    mShift.setStyle(Paint.Style.STROKE);
    mShift.setStrokeWidth(10);

    mShiftFill = new Paint();
    mShiftFill.setColor(Color.parseColor("#0EFFFF"));
    mShiftFill.setStyle(Paint.Style.FILL);
  ...
  private Bitmap getSrc() {
    ...
    switch (shiftGear){
      case 0 : shiftGearString = "N";
        break;
      case -1: shiftGearString = "R";
        break;
      default: shiftGearString = String.valueOf(shiftGear);
        break;
    }
    canvas.drawRect(0,0, mWidth, mHeight, clearPaint);
    if (shiftGear != shiftGearPrev){
      canvas.drawRect(centerX-60,centerY-60,centerX+60,centerY+60,mShiftFill);
      canvas.drawRect(centerX-60,centerY-60,centerX+60,centerY+60,mShift);
    }else{
      canvas.drawRect(centerX-50,centerY-50,centerX+50,centerY+50,mShiftFill);
      canvas.drawRect(centerX-50,centerY-50,centerX+50,centerY+50,mShift);
      canvas.drawText(shiftGearString, centerX-15,centerY+15, mTextPaint);
    }
    shiftGearPrev = shiftGear;
  ...
  protected void onDraw(Canvas canvas) {
    ...
    canvas.drawText("Angle: " + mRoll, mAngleChordX, mAngleChordY, mTextPaint);
